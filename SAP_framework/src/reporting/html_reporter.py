...
class HTMLReporter:
    def __init__(self, report_dir=None):
        # ✅ NUEVO: Soporte para rutas modulares
        ...
        # ✅ NUEVO: Directorio para evidencias
    ...
    def _analyze_scenario_detailed(self, scenario):
        ...
        if 'steps' not in scenario:
            return self._create_scenario_structure(scenario, status, total_duration, steps_details, start_time, end_time)
        ...
        for step in scenario['steps']:
            ...
            # ✅ NUEVO: Capturar detalles completos del paso
            step_detail = {
                'name': step.get('name', ''),
                'keyword': step.get('keyword', ''),
                'status': step_status,
                'duration': round(step_duration, 3),  # ✅ Precisión de milisegundos
                ...
                'evidence': self._generate_step_evidence(step, step_status)  # ✅ Evidencias
            }
            steps_details.append(step_detail)
        
        return self._create_scenario_structure(scenario, status, round(total_duration, 3), steps_details, start_time, end_time)
    ...
    def _capture_screenshot(self, step_name):
        ...
        try:
            # ✅ PARA IMPLEMENTAR: Integración con SAP GUI para screenshots
            # Por ahora retorna información de placeholder
    ...
    def generate_html_report(self, json_report_path):
        ...
        report_path = os.path.join(self.report_dir, f"sap_test_report_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.html")
        ...
        # ✅ NUEVO: Generar reporte adicional en JSON para análisis
